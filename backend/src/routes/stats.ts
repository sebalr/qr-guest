...
router.get('/:id/device-debug-data', async (req: Request, res: Response): Promise<void> => {
	...
	const rows = await withRls(context, async tenantPrisma => {
...
router.get('/:id/stats', async (req: Request, res: Response): Promise<void> => {
	...
	const result = await withRls(context, async tenantPrisma => {
		...
		if (interval === '30m') {
			scansByIntervalRaw = await tenantPrisma.$queryRaw<{ bucket: string; count: bigint }[]>`
    ...
    `;
		...
		const [totalGuests, totalScans, uniqueTicketsResult, topGuestsRaw, userScanRankingRaw, duplicateTicketsRaw, firstScansByIntervalRaw] =
			await Promise.all([
				tenantPrisma.ticket.count({ where: { eventId } }),
				tenantPrisma.scan.count({ where: { eventId } }),
				tenantPrisma.scan.groupBy({ by: ['ticketId'], where: { eventId } }),
				// Top 10 guests by scan count (includes not-yet-scanned).
				tenantPrisma.$queryRaw<{ ticket_id: string; name: string; scan_count: bigint }[]>`
    ...
    `,
				...
				tenantPrisma.$queryRaw<{ user_id: string; email: string; scan_count: bigint }[]>`
    ...
    `,
				...
				interval === '30m'
					? tenantPrisma.$queryRaw<{ bucket: string; count: bigint }[]>`
	...
	res.json({
		data: {
			...result,
			scansByHour: result.scansByInterval,
		},
	});
});
