...
describe("billing calculations and signatures", () => {
  it("rounds total in decimal ARS cents", () => {
    expect(quoteAmount(3, "1234.567890").toFixed(2)).toBe("2407.41");
    expect(quoteAmount(1, "1000.05").toFixed(2)).toBe("650.03");
  });
  it("charges roughly ARS 100,000 for 100 paid QRs at the reference rate", () => {
    expect(quoteAmount(100, "1530").toFixed(2)).toBe("99450.00");
  });
