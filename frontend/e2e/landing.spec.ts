...
test("pricing simulator, translation and responsive landing", async ({
  ...
  await page.goto("/");
  await expect(page.getByTestId("estimate-usd")).toHaveText("USD 15.00");
  await expect(page.getByTestId("estimate-ars")).toHaveText("ARS 15,000.00");
  ...
  await expect(page.getByTestId("estimate-usd")).toHaveText("USD 5.00");
  await page.getByLabel("How many guests?", { exact: true }).fill("0");
  await expect(page.getByRole("alert")).toBeVisible();
  await expect(page.getByTestId("estimate-usd")).toHaveText("—");
  await page.getByLabel("How many guests?", { exact: true }).fill("200");
