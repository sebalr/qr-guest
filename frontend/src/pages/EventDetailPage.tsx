...
export default function EventDetailPage() {
  ...
  async function handleShowQR(ticketId: string) {
    if (qrMap[ticketId]) {
      setQrMap((prev) => { const n = { ...prev }; delete n[ticketId]; return n; });
      return;
    }
    try {
      const res = await getTicketQRApi(ticketId);
      setQrMap((prev) => ({ ...prev, [ticketId]: res.data.data }));
    } catch {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-blue-700 text-white px-6 py-4 flex items-center gap-4 shadow">
        <button onClick={() => navigate('/events')} className="text-white opacity-80 hover:opacity-100">
          ← Back
        </button>
      ...
      <main className="max-w-3xl mx-auto px-4 py-6">
        ...
        {showBulk && (
          <form onSubmit={handleAddTickets} className="bg-white rounded-xl shadow p-5 mb-5 space-y-3">
        ...
        <ul className="space-y-3">
          {tickets.map((ticket) => (
            <li key={ticket.id} className="bg-white rounded-xl shadow p-4">
